...
| B | **中转站兼容** | deepagents 经 `ChatOpenAI`（`langchain-openai`）连 `one.zhique.cn/v1` + `deepseek-v4-flash`，能完成带 `bind_tools` 的 tool-calling（中转站对 OpenAI tool-calling 协议的兼容） | 决策 3、CLAUDE.md「LLM 与 Embedding 统一走中转站」 |
