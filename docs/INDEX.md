...
> 当前项目是大学校园多智能体平台。核心业务之一是公选课推荐：系统把学生自然语言选课需求转成结构化画像和硬约束，用 MySQL、Milvus、Redis 从 500 门真实课程中召回候选，经过确定性硬约束过滤和 LLM 语义初筛后做候选内重排，最后生成可解释推荐理由。编排支持固定 Pipeline 和 ReAct 工具调用两种模式，通过 A/B 实验分流。平台还提供学生手册/成绩单知识库问答、成绩报告、评价寄语等校园业务。核心思路是规则保下限、LLM 提上限。
