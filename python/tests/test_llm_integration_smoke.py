...
默认跳过（不占额度）。仅在显式开启时运行：
  ...
  # Windows PowerShell
  $env:E2E_LLM="1"; pytest tests/test_llm_integration_smoke.py -m integration -v

  # bash
  E2E_LLM=1 pytest tests/test_llm_integration_smoke.py -m integration -v

需先在环境中配置 LLM_API_KEY、LLM_BASE_URL、LLM_MODEL（或通过 .env）。
"""
...
async def test_openai_compatible_llm_smoke_when_e2e_enabled():
    if os.environ.get("E2E_LLM", "").strip() != "1":
        pytest.skip(
            "默认跳过以避免计费；设置 E2E_LLM=1 且配置 LLM_* 后运行本测试。"
        )
