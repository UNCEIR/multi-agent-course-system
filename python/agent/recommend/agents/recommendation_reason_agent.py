...
4. 抢课优先权：大四>大三>大二=大一。低年级学生选爆满课抢到概率低，理由里须结合年级提示风险。
5. 不要编造数据，只能使用输入字段。
6. 每条 40-80 字。
...
5. 抢课优先权：大四>大三>大二=大一。低年级学生选爆满课抢到概率低，理由里须结合年级提示风险。
6. 如果课程爆满、容量紧张、考核方式不完全匹配，在推荐理由中温和提示风险，自然融入文本。
7. 不要编造数据，只能使用输入字段。
8. 不要输出 JSON，只输出纯文本。
9. 所有课程推荐完成后直接结束，不需要总结段。
